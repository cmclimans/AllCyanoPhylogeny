library(dplyr)
library(stringr)
outputs = T # Condition to write output files (F = no outputs, T = write outputs)

# Import records file for processing
...
if (nrow(check_dates) == 0) {
  ...
  if(outputs == T & 
     file.exists("~/Desktop/genome_records_tocheck.csv") == F ){write.csv(check_dates, file = 'genome_records_tocheck.csv')}
...
if (outputs == T & file.exists("~/Desktop/genomes_clean_parsed.csv") == FALSE){
  write.csv(genome_records_parse, file = '~/Desktop/genomes_clean_parsed.csv', row.names = FALSE)}
...
if (outputs == T & file.exists("~/Desktop/rename.csv") == F){
  write.csv(rename_file, file = '~/Desktop/rename.csv', row.names = FALSE)}
